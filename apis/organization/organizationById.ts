...
export const getJoinRequestsByOrg = async (
  req: IGetJoinRequestsRequest,
): Promise<IGetJoinRequestsResponse> => {
  const { organization_id, ...rest } = req;
  return await requestApi.get<IGetJoinRequestsResponse>(`${url}/${organization_id}/join-requests`, rest);
};

export const useGetJoinRequestsByOrg = (
  req: IGetJoinRequestsRequest,
  options?: Omit<
    UseGetOptions<IGetJoinRequestsResponse>,
    "queryKey" | "queryFn"
  >,
) => {
  return useGet({
    ...
    queryFn: () => getJoinRequestsByOrg(req),
    ...options,
  });
};
