...
import { useCreateAdminSeason, usePatchAdminSeason } from '@/apis/gamification/season/list';
...
import { parseIsoToDate, toIsoEndOfDay, toIsoStartOfDay } from '../_services/seasonAdmin.service';
...
function validateSeasonDates(values: SeasonFormValues): {
  ok: boolean;
  startsAtIso?: string;
  endsAtIso?: string;
  errorField?: 'startsAt' | 'endsAt';
  errorMessage?: string;
} {
  if (!values.startsAt || !values.endsAt) {
    return { ok: false, errorField: 'startsAt', errorMessage: 'Date range is required' };
  }
  if (values.startsAt >= values.endsAt) {
    return { ok: false, errorField: 'endsAt', errorMessage: 'End date must be after start date' };
  }

  const startsAtIso = toIsoStartOfDay(values.startsAt);
  const endsAtIso = toIsoEndOfDay(values.endsAt);
  if (!startsAtIso || !endsAtIso) {
    return { ok: false, errorField: 'startsAt', errorMessage: 'Invalid dates' };
  }

  return { ok: true, startsAtIso, endsAtIso };
}
...
export const CreateUpdateSeason = memo(function CreateUpdateSeason({
  ...
  const onValidSubmit = useCallback(
    async (values: SeasonFormValues) => {
      ...
      const dateValidation = validateSeasonDates(values);
      if (!dateValidation.ok) {
        setError(dateValidation.errorField ?? 'startsAt', {
          message: t(dateValidation.errorMessage ?? 'Invalid dates'),
        });
        return;
      }
      ...
      try {
        if (isCreate) {
          await createMutate({
            ...
            kind: values.kind,
            startsAt: dateValidation.startsAtIso!,
    ...
    [clearErrors, createMutate, isCreate, patchMutate, season, setError, t],
  );
  ...
  return (
    ...
    >
      <DialogContent
        showCloseButton
        className={cn('max-w-lg gap-4 overflow-y-auto sm:max-w-xl scrollbar-hide')}
      >
          ...
          <Field>
            ...
            <div className="relative">
              <Button
                ...
                variant="outline"
                className={cn(
                  '!h-10 w-full justify-start text-left font-normal !border !border-zinc-300 hover:bg-transparent',
