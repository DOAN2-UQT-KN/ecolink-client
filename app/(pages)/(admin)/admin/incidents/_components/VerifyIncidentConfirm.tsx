...
export const VerifyIncidentConfirm = memo(function VerifyIncidentConfirm({
  ...
  const { mutateAsync: verifyAsync } = useVerifyReport({
    queryKey: ["incidents"],
    messageSuccess: { type: MessageType.Toast },
  });

  const { mutateAsync: banAsync } = useBanReport({
    queryKey: ["incidents"],
    messageSuccess: { type: MessageType.Toast },
  });
  ...
  const handleConfirm = useCallback(async () => {
    ...
    if (shouldBan) {
      ...
      setPending(true);
      try {
        await banAsync({ id: incidentId, reject_reason: reason });
        await queryClient.invalidateQueries({ queryKey: ["incidents"] });
        showMessage({
          type: MessageType.Toast,
          level: MessageLevel.Success,
          title: t("Report banned successfully"),
        });
        resetForm();
      } finally {
        setPending(false);
  ...
  }, [banAsync, banReason, decision, incidentId, isBanMode, resetForm, t, verifyAsync]);
  ...
  return (
    <ConfirmPopover
      theme={theme}
      title={isBanMode ? t("Ban this incident?") : t("Verify this incident?")}
      description={
        isBanMode
          ? t("This will ban {{name}}.", { name: incidentTitle })
          : t("You can verify {{name}} or ban it.", { name: incidentTitle })
      }
      ...
      extraContent={
        <div className="space-y-4">
          {!isBanMode ? (
            ...
            >
              <label
                htmlFor={`incident-decision-approve-${incidentId}`}
                className="flex cursor-pointer items-center gap-2 text-sm"
              >
                <RadioGroupItem
                  value="approve"
                  id={`incident-decision-approve-${incidentId}`}
                />
                {t("Verify")}
              </label>
              <label
                htmlFor={`incident-decision-ban-${incidentId}`}
                className="flex cursor-pointer items-center gap-2 text-sm"
              >
                <RadioGroupItem
                  value="ban"
                  id={`incident-decision-ban-${incidentId}`}
                />
                {t("Ban")}
              </label>
            </RadioGroup>
